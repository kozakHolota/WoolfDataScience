...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char in self.op_table.keys():
                token = self.op_table[self.current_char]  # спочатку зберігаємо токен оператора
                self.advance()
                return token
...
class Parser:
    ...
    def term(self):
        node = self.factor()
        while self.current_token.type in (TokenType.MULTIPLE, TokenType.DIVIDE):
            token = self.current_token
            if token.type == TokenType.MULTIPLE:
                self.eat(TokenType.MULTIPLE)
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            return self.visit(node.left) - self.visit(node.right)
        elif node.op.type == TokenType.MULTIPLE:
            return self.visit(node.left) * self.visit(node.right)
